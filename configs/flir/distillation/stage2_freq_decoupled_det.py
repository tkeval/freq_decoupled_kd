# -------------------------------------------------------------------------
# Stage 2: Detection Fine-Tuning with Freq-Decoupled Stage 1 Backbone (FLIR)
# -------------------------------------------------------------------------
# Adapted from KAIST stage2_freq_decoupled_det.py for FLIR dataset.
# Uses KAIST Stage 1 checkpoint — the LoRA adapters learned modality-general
# (low-freq) adaptations that should transfer across IR datasets.
#
# Usage:
#   bash ./tools/dist_train.sh configs/flir/distillation/stage2_freq_decoupled_det.py 4 \
#       --work-dir ./work_dirs/flir_stage2_freq_decoupled
# -------------------------------------------------------------------------
_base_ = [
    '../dinov2/dino-5scale_dinov2-large-p14-reg4_8xb2-12e_flir.py'
]

# -------------------------------------------------------------------------
# 1. Stage 1 Checkpoint (KAIST freq-decoupled KD, reused for FLIR)
# -------------------------------------------------------------------------
stage1_checkpoint = './work_dirs/stage1_freq_decoupled/epoch_12.pth'
...
model = dict(
    ...
    # --- Student Detector (inherited from base FLIR DINO config) ---
...
# -------------------------------------------------------------------------
work_dir = './work_dirs/flir_stage2_freq_decoupled'
