# -------------------------------------------------------------------------
# Single-Stage Cross-Modal Detector KD: RGB Teacher → IR Student
# -------------------------------------------------------------------------
# A pre-trained RGB DINO detector (teacher) guides an IR DINO student
# with combined detection + feature + response distillation losses.
#
# Loss = Det_Loss(IR_student, GT)
#      + λ_feat * Feature_KD(student_neck, teacher_neck)
#      + λ_cls  * Response_KD_Cls(student_logits, teacher_logits)
#      + λ_bbox * Response_KD_BBox(student_boxes, teacher_boxes)
#
# Key differences from 2-stage approach:
#   - Single stage: backbone is optimized for detection from the start
#   - Teacher is a full RGB DINO detector (not just a backbone)
#   - KD serves as regularizer, not the primary objective
#
# Usage:
#   bash ./tools/dist_train.sh configs/kaist/distillation/cross_modal_detector_kd_dino.py 4 \
#       --work-dir ./work_dirs/cross_modal_detector_kd
# -------------------------------------------------------------------------
_base_ = [
...
# -------------------------------------------------------------------------
# 1. RGB Teacher Checkpoint (UPDATE THIS PATH)
# -------------------------------------------------------------------------
...
# -------------------------------------------------------------------------
# 3. Student Model: CrossModalDetectorKD (extends DINO + adds KD)
# -------------------------------------------------------------------------
model = dino_model_cfg.copy()
model['type'] = 'CrossModalDetectorKD'
model['teacher_cfg'] = dino_model_cfg
model['teacher_checkpoint'] = teacher_checkpoint
model['distill_cfg'] = dict(
    loss_weight_feat=0.01,   # Light weights (~0.05 summed over 5 neck scales)
    loss_weight_cls=0.05,
    loss_weight_bbox=0.05,
    temperature=2.0)
...
work_dir = './work_dirs/cross_modal_detector_kd'
